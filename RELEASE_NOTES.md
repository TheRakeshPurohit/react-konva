# Unreleased: native input synchronization

This candidate uses the public event hook in published Konva 10.5.0.
Older Konva versions remain accepted. The react-konva package version stays
unchanged until the maintainer chooses the release version.

## Changes

- With the new hook, native Konva input commits ordinary React state before each Konva listener returns, including state owned above Stage. Controlled Transformer geometry is current before the next frame.
- Handlers within one native listener share a batch. The integration uses React DOM `flushSync` and the custom renderer's host scheduler. It does not inspect hidden React DOM fields or replace Konva input methods.
...
dispatch from an unrelated React effect must be queued after that effect; see
...
DOM elements. The timing runner and measurements are in [benchmarks](benchmarks/README.md).
